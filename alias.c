...
{
	alias_t *node, **temp1 = &args->alias_link;
	char *alias, *val_string, **val;
	char *cp1 = _strdup(str);

	alias = strtok(cp1, "=");
	val_string = strtok(NULL, "");
	val = get_alias_val(val_string);
	node = locate_alias(args, alias);
	if (node == NULL)
	{
		if (add_alias_node(temp1, alias, val, str) == NULL)
			perror(args->exe);
	}
	else
	{
